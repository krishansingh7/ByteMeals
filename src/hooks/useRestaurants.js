...
const useRestaurants = () => {
  ...
  const fetchMoreRestaurants = () => {
    ...
    setTimeout(() => {
      // Create a fresh batch by cloning existing restaurants and ensuring keys are uniquely randomized
      const newBatch = allRestaurants.map(restaurant => {
        // Deep copy the original restaurant item safely using JSON serialization
        const clonedRestaurant = JSON.parse(JSON.stringify(restaurant));
        
        // Add a random hash to the unique ID so React doesn't complain about duplicate DOM keys
