  ...
  const handleOAuthLogin = async (providerName) => {
    ...
    try {
      ...
      if (error.code === 'auth/account-exists-with-different-credential') {
        toast.error('Account exists with a different provider. Use Google or Email instead.');
      } else {
        toast.error(error.message);
      }
    } finally {
    ...
    <div className="flex min-h-[80vh] items-center justify-center p-4">
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-md">
        ...
        <div className="flex justify-between items-center mb-2">
           ...
           <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto/Image-login_btpq7r" alt="Login Food" className="w-20 h-20 object-contain rounded-full border border-slate-100 p-1" />
        </div>
        
        <p className="mb-6 text-sm text-gray-600">
          or <Link to="/signup" className="text-orange-500 font-bold hover:text-orange-600">create an account</Link>
        </p>
